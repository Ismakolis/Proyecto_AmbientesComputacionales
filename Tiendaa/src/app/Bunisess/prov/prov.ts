...
export class Prov implements OnInit {
  ...
  obtenerProveedores(): void {
    this.http.get<any[]>('http://localhost:3000/api/proveedores')
      .subscribe(data => {
  ...
  guardarProveedor(): void {
    ...
    if (this.editando) {
      this.http.put(`http://localhost:3000/api/proveedores/${this.proveedorEditandoId}`, data)
        .subscribe(() => {
          this.obtenerProveedores();
          this.cancelarEdicion();
        });
    } else {
      this.http.post('http://localhost:3000/api/proveedores', data)
        .subscribe(() => {
          this.obtenerProveedores();
